...
import {
  ...
  fetchMaterialConsumption, uploadProfilePhoto, updateEmployee, updateTeamLeader,
  supabaseUpdatePassword, supabaseVerifyPassword,
...
const SessionManager = (() => {
  ...
  const reset = () => {
    ...
    if (onExpire) {
      timer = setTimeout(() => {
        try { onExpire(); } catch (e) { console.error("Session expiry error:", e); }
      }, SESSION_TIMEOUT_MS);
    }
  };
  return {
    start(cb) {
      ...
      // Remove click from reset to avoid errors on every click
      reset();
    },
    stop() {
      isActive = false;
...
const TestRunner = (() => {
  ...
  const expect = (val) => {
    ...
    const notMatchers = {
      toBe: (exp) => { if (val === exp) throw new Error(`Expected NOT ${JSON.stringify(exp)}, got ${JSON.stringify(val)}`); },
      toEqual: (exp) => { if (JSON.stringify(val) === JSON.stringify(exp)) throw new Error(`Expected NOT ${JSON.stringify(exp)}, got ${JSON.stringify(val)}`); },
      toBeTruthy: () => { if (!!val) throw new Error(`Expected falsy, got ${JSON.stringify(val)}`); },
      toBeFalsy: () => { if (!val) throw new Error(`Expected truthy, got ${JSON.stringify(val)}`); },
      toContain: (exp) => { if (Array.isArray(val) ? val.includes(exp) : String(val).includes(exp)) throw new Error(`Expected NOT to contain ${exp}`); },
      toMatch: (rx) => { if (rx.test(String(val))) throw new Error(`Expected NOT to match ${rx}, got ${val}`); },
      toBeGreaterThan: (n) => { if (val > n) throw new Error(`Expected ${val} NOT > ${n}`); },
      toBeLessThan: (n) => { if (val < n) throw new Error(`Expected ${val} NOT < ${n}`); },
      toThrow: () => {
        if (typeof val !== "function") throw new Error("toThrow needs a function");
        try { val(); return; }
        catch (e) { throw new Error("Expected function NOT to throw"); }
      },
    };
    const chain = Object.assign({}, matchers);
    Object.defineProperty(chain, "not", { get: () => Object.assign({}, notMatchers) });
    return chain;
  };
...
// ─── Data ────────────────────────────────────────────────────────────────────
const CREDENTIALS = [
  { email: "admin@corp.com", password: "Admin@1234", role: "admin", id: "emp-001" },
  { email: "m.webb@corp.com", password: "Marcus@1234", role: "employee", id: "emp-002" },
];
const MOCK_EMPLOYEES = [
  { email: "admin@corp.com", name: "Sarah Mitchell", role: "admin", id: "emp-001", department: "Administration" },
  { email: "m.webb@corp.com", name: "Marcus Webb", role: "employee", id: "emp-002", department: "IT" },
];
const SEED_PROJECTS = [];
...
// Helper to check if value is a valid image URL
const isImageUrl = (value) => {
  if (!value || typeof value !== "string") return false;
  const v = value.trim();
  // Support data URLs that might be slightly corrupted (e.g. have a leading comma)
  return v.startsWith("http") || v.startsWith("blob:") || v.includes("data:image/");
};

const Avt = ({ initials, name, size = 32, colors = ["#2563eb", "#3b82f6"] }) => {
  const trimmed = typeof initials === "string" ? initials.trim() : "";
  // Search for the data: part in case there's leading garbage/commas
  const dataIndex = trimmed.indexOf("data:image/");
  const cleanUrl = dataIndex !== -1 ? trimmed.substring(dataIndex) : trimmed;
      ...
      <div style={{
        ...
        <img
          ...
          onError={(e) => {
            ...
            // Show initials only if the image fails
            const p = e.currentTarget.parentElement;
            if (p) p.style.background = `linear-gradient(135deg,${colors[0]},${colors[1]})`;
          }}
        />
      </div>
    );
  ...
  // Ensure we don't display long base64 strings as text initials
  const displayInitials = (trimmed && trimmed.length < 10 && !trimmed.includes(';')) ? trimmed : getInitials(name);
...
const calculateProjectProgress = (project, reports, projectItems) => {
  ...
  const rawCompletedQty = projReports.filter(r => {
    if (!r.projectItemId) return true; // General scope reports always count
    const item = (projectItems || []).find(it => it.id === r.projectItemId);
    if (!item) return false;

    const u1 = (item.unit || "").toLowerCase().trim();
    const u2 = (project.unitType || "").toLowerCase().trim();
    // Fuzzy match common units: mtr, metr, meter, meters -> all same
    const isMeter = (u) => u === "meter" || u === "meters" || u === "mtr" || u === "metr";
    const isKg = (u) => u === "kg" || u === "kgs" || u === "kilogram" || u === "kilograms";

    if (isMeter(u1) && isMeter(u2)) return true;
    if (isKg(u1) && isKg(u2)) return true;
    return u1 === u2;
  }).reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0);
  ...
  return { completedQty, totalQty, remaining, progress, rawCompletedQty };
};
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", padding: "clamp(12px, 3vw, 24px)"
    }}>
      ...
      <div className="d-flex flex-column flex-md-row w-100" style={{
        position: "relative", zIndex: 1, background: C.white, borderRadius: 28,
        boxShadow: "0 12px 60px rgba(37,99,235,.15)", overflow: "hidden",
        maxWidth: "min(960px, 100%)", minHeight: "min(580px, 90vh)"
      }}>

        {/* Left panel */}
        <div className="d-none d-md-flex flex-column justify-content-center" style={{
          width: "clamp(260px, 40%, 420px)", background: "linear-gradient(145deg,#1d4ed8 0%,#2563eb 50%,#60a5fa 100%)",
          padding: "clamp(24px, 5vw, 52px) clamp(20px, 4vw, 48px)", position: "relative", overflow: "hidden"
        }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={{ marginBottom: 36 }}>
              ...
              <h1 style={{ fontSize: "clamp(24px, 3vw, 32px)", fontWeight: 900, color: "#fff", lineHeight: 1.2, marginBottom: 12, letterSpacing: "-.02em" }}>
              ...
              <p style={{ fontSize: "clamp(12px, 1.2vw, 13px)", color: "rgba(255,255,255,.75)", lineHeight: 1.7, maxWidth: 320, marginBottom: 28 }}>
                {mode === "login"
        ...
        {/* Right panel — Form */}
        <div className="flex-fill d-flex flex-column justify-content-center fu" style={{ padding: "clamp(24px, 5vw, 44px) clamp(20px, 4vw, 48px)", overflowY: "auto" }}>
      ...
      <div style={{ padding: "14px 14px", borderTop: `1px solid ${C.border}` }}>
        <div style={{ background: C.bluePale, borderRadius: 16, padding: "12px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            <Avt initials={user?.avatar} name={user?.name} size={34} />
...
// ─── Bottom Nav ───────────────────────────────────────────────────────────
function BottomNav({ role, active, onNav, unreadAnnouncements }) {
  const adminNav = [
    { k: "overview", l: "Dash", ico: "⊞" },
    { k: "projects", l: "Proj", ico: "📊" },
    { k: "employees", l: "Emp", ico: "👥" },
    { k: "reports", l: "Rep", ico: "📋" },
  ];
  const empNav = [
    { k: "submit", l: "Submit", ico: "✏️" },
    { k: "history", l: "Hist", ico: "📄" },
    { k: "announcements", l: "Announce", ico: "📢" },
  ];
  const tlNav = [
    { k: "tl-submit", l: "Submit", ico: "✏️" },
    { k: "tl-projects", l: "Proj", ico: "🏗️" },
    { k: "tl-history", l: "Hist", ico: "📄" },
    { k: "tl-announce", l: "Ann", ico: "📢" },
  ];
  ...
  return (
    <div className="bottom-nav" style={{
      ...
      {navItems.map(item => {
        const isA = active === item.k;
        return (
          <button key={item.k} onClick={() => onNav(item.k)} style={{
            background: "transparent", border: "none", display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center", flex: 1, height: "100%", color: isA ? C.blue : C.muted
          }}>
            <span style={{ fontSize: 20, marginBottom: 2 }}>{item.ico}</span>
            <span style={{ fontSize: 10, fontWeight: 700 }}>{item.l}</span>
          </button>
        );
      })}
    </div>
...
// ─── Top bar ──────────────────────────────────────────────────────────────────
function TopBar({ title, sub, user, onLogout, announcements = [], setEmployees, profilePhotoPreview, setProfilePhotoPreview }) {
  const [open, setOpen] = useState(false);
  const [profileOpen, setProfileOpen] = useState(false);
  const [notifs, setNotifs] = useState([]);
  const [modal, setModal] = useState(null); // "profile" | "settings" | "password"

  // Memoize announcements string to prevent unnecessary effect triggers
  const announcementsKey = useMemo(() =>
  ...
  useEffect(() => {
    if (announcements && announcements.length > 0) {
      ...
      const relevant = announcements.filter(a => {
        const isRecipient = (a.recipientIds || []).includes(userId);
        const isDept = a.department === userDept;
  ...
  }, [announcementsKey, userId, userDept]); // Use stable keys instead of object references

  const unread = notifs.filter(n => !n.read).length;

  // Profile states are now destructured from TopBar arguments

  // Profile edit state
  const [profileForm, setProfileForm] = useState({ name: user?.name || "", email: user?.email || "", phone: "", department: user?.department || "" });
  const [profileOpenInner, setProfileOpenInner] = useState(false); // separate from the modal
  const [profileSaved, setProfileSaved] = useState(false);
  const [profilePhoto, setProfilePhoto] = useState(null);
  const [localPhotoPreview, setLocalPhotoPreview] = useState(null);
  const [uploadingPhoto, setUploadingPhoto] = useState(false);

  const currentPreview = profilePhotoPreview || localPhotoPreview;
  ...
  const handleProfilePhotoChange = (e) => {
    ...
    if (file) {
      ...
      if (typeof setProfilePhotoPreview === "function") {
  ...
  const saveProfile = async () => {
    try {
      let photoUrl = currentPreview;

      // Upload photo if a new one was selected (blob: or data: URL)
      if (profilePhoto && (currentPreview?.startsWith('blob:') || currentPreview?.startsWith('data:'))) {
        setUploadingPhoto(true);
        try {
          photoUrl = await uploadProfilePhoto(profilePhoto, user?.id || "default");
          console.log("Photo uploaded, result:", photoUrl ? "success" : "empty");
        } catch (uploadError) {
      ...
      // Update employee record in database with avatar URL
      if (photoUrl && (photoUrl.startsWith('data:') || photoUrl.startsWith('http'))) {
        console.log("Updating employee avatar in database...");
        try {
          await updateEmployee(user?.id, { avatar: photoUrl });
          console.log("Employee updated successfully");
        } catch (updateError) {
          console.error("Failed to update employee:", updateError);
          alert("Failed to save profile photo to database. Please try again.");
          return;
        }

        // Update local employees state to reflect changes immediately
        setEmployees(prev => prev.map(emp =>
          emp.id === user?.id ? { ...emp, avatar: photoUrl } : emp
        ));

        // Force refresh of employees cache to get updated data
        try {
          const freshEmployees = await fetchEmployees({ forceRefresh: true, useCache: false });
          if (freshEmployees && freshEmployees.length > 0) {
            setEmployees(freshEmployees);
            console.log("Employees cache refreshed, count:", freshEmployees.length);
          }
        } catch (refreshError) {
          console.warn("Failed to refresh employees cache:", refreshError);
          // Continue anyway - local state is already updated
        }
      }

      // Update user avatar URL in session/storage if needed
      if (photoUrl) {
        // Store the photo URL for future sessions
        sessionStorage.setItem('userAvatarUrl', photoUrl);
      }
  ...
  const savePassword = async () => {
    ...
    try {
      const verifyRes = await supabaseVerifyPassword(user?.email, pwForm.current);
      if (!verifyRes.ok) {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ position: "relative" }}>
            <button id="profile-btn" onClick={() => { setProfileOpen(o => !o); setOpen(false); }} style={{
              ...
              <Avt initials={currentPreview || user?.avatar} name={user?.name} size={38} />
            </button>
              ...
              <div id="profile-panel" style={{
                ...
                <div style={{ padding: "20px 18px 16px", background: `linear-gradient(135deg,${C.blue},${C.blueL})`, textAlign: "center" }}>
                  <div style={{
                    ...
                    <Avt initials={currentPreview || user?.avatar} name={user?.name} size={56} />
                  </div>
        ...
        modal === "profile" && (
          <Modal titleText="👤 My Profile" onClose={() => setModal(null)}>
            ...
            <div style={{
              ...
              padding: "20px", background: C.bluePale, borderRadius: 16
            }}>
              <div style={{
                ...
                <Avt initials={currentPreview || user?.avatar} name={user?.name} size={80} />
              </div>
              <label style={{
                cursor: "pointer",
                padding: "8px 16px",
            ...
            <div style={{
              ...
              <div style={{
                ...
                <Avt initials={currentPreview || user?.avatar} name={user?.name} size={60} />
              </div>
              <div>
...
// ─── Admin Overview ───────────────────────────────────────────────────────────
function AdminOverview({ onNavigate, user, projects, reports, projectItems, employees = [], onStatusChange, onEditProject, onLogout, announcements = [], setEmployees, profilePhotoPreview, setProfilePhotoPreview }) {
      ...
      <W cls="fu4" style={{ marginBottom: 20 }}>
        ...
        <div style={{ padding: "16px 22px" }}>
          {allProjects.filter(p => p.totalWorkQty).map((p, i) => {
            const { completedQty, totalQty, progress } = calculateProjectProgress(p, allReports, projectItems);
                    ...
                    <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
                      {(() => {
                        ...
                        const unlinkedRender = unlinkedQty > 0 ? (
                          <div style={{ display: "flex", alignItems: "center", gap: 8, opacity: 0.9, background: "#f8faff", padding: "4px 8px", borderRadius: 8, border: `1.1px solid ${C.border}` }}>
                            <span style={{ fontSize: 10, fontWeight: 700, color: C.muted, minWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>📦 General Project Scope</span>
                            <div style={{ flex: 1, height: 5, background: "#e2e8f0", borderRadius: 5 }}>
                              <div style={{ height: "100%", width: "100%", borderRadius: 5, background: "#60a5fa", opacity: 0.5 }} />
                            </div>
                            <span style={{ fontSize: 9, fontWeight: 800, color: C.muted, minWidth: 70, textAlign: "right" }}>
...
// ─── Reports List ──────────────────────────────────────────────────────────────
function ReportsList({ onSelect, user, onLogout, reports, employees, projects, announcements = [], setEmployees, profilePhotoPreview, setProfilePhotoPreview }) {
  const allReports = reports || [];
  const allEmployees = employees || MOCK_EMPLOYEES;
  const allProjects = projects || MOCK_PROJECTS;
  const [f, setF] = useState({ dept: "", project: "", employee: "", date: "", type: "" });

  // Count off-site reports
  const offSiteCount = allReports.filter(r => r.isOffSite).length;
  ...
  const filtered = allReports.filter(r => {
    ...
    if (f.type === "regular" && r.isOffSite) return false;
    if (f.type === "offsite" && !r.isOffSite) return false;
    return true;
  });

  return (
    <div>
      <TopBar title="Work Reports" sub={`${filtered.length} records · AI-processed intelligence`} user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />

      {/* Off-site alert banner */}
      {offSiteCount > 0 && (
        <div style={{
          marginBottom: 16, padding: "12px 18px", background: "#f3e8ff", borderRadius: 12,
          border: "1.5px solid #d8b4fe", display: "flex", alignItems: "center", justifyContent: "space-between",
          flexWrap: "wrap", gap: 10
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 18 }}>🌍</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#7c3aed" }}>
              {offSiteCount} Off-Site / Ad-hoc {offSiteCount === 1 ? "report" : "reports"} in the system
            </span>
          </div>
          {f.type !== "offsite" && (
            <button onClick={() => setF(p => ({ ...p, type: "offsite" }))} style={{
              padding: "6px 14px", background: "#7c3aed", color: "#fff", border: "none",
              borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer"
            }}>
              Filter Off-Site Only
            </button>
          )}
        </div>
      )}
      ...
      <W style={{ marginBottom: 16, padding: "14px 18px" }} cls="fu1">
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          {[
            ...
            { k: "type", lbl: "All Types", opts: [{ v: "regular", l: "🏗 Regular" }, { v: "offsite", l: "🌍 Off-Site" }] },
          ].map(fl => (
          ...
          <button onClick={() => setF({ dept: "", project: "", employee: "", date: "", type: "" })} style={{
...
// ─── Report Detail ─────────────────────────────────────────────────────────────
function ReportDetail({ report, onBack, user, onLogout, employees = [], projects = [], announcements = [], setEmployees }) {
  const emp = (employees.length ? employees : MOCK_EMPLOYEES).find(e => e.id === report.employeeId);
  const proj = (projects.length ? projects : MOCK_PROJECTS).find(p => p.id === report.projectId);
  const isOffSite = report.isOffSite;

  return (
    <div>
      <div style={{ marginBottom: 18 }}><Btn v="ghost" onClick={onBack} icon="←" sm>Back to Reports</Btn></div>
      <TopBar title="Report Detail" user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />

      {/* Off-site banner */}
      {isOffSite && (
        <div style={{
          marginBottom: 16, padding: "14px 18px", background: "#f3e8ff", borderRadius: 14,
          border: "2px solid #d8b4fe", display: "flex", alignItems: "center", gap: 14
        }}>
          <div style={{ fontSize: 28 }}>🌍</div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#7c3aed", marginBottom: 4 }}>Off-Site / Ad-hoc Work Report</div>
            <div style={{ fontSize: 12, color: C.muted, fontWeight: 600 }}>
              <strong>Work Type:</strong> {report.offSiteWorkType} &nbsp;|&nbsp;
              <strong>Location:</strong> {report.offSiteLocation}
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <W cls="fu" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: C.light, marginBottom: 14 }}>
            {isOffSite ? "🌍 Employee Record (Off-Site)" : "Employee Record"}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18 }}>
            <Avt initials={emp?.avatar} name={emp?.name} size={48} />
            <div>
          ...
          {isOffSite ? [
            ...
            ["Report Type", "🌍 Off-Site"],
            ["Work Type", report.offSiteWorkType],
            ["Location", report.offSiteLocation],
            ["Purpose", report.offSitePurpose],
            ["Date", report.date],
            ["Hours Worked", `${report.hours} hrs`],
            ["Manpower", `${report.manpowerCount || "—"} people`],
          ] : [
            ["Department", emp?.department],
            ["Project", proj?.name],
            ["PO Number", proj?.poNumber || "—"],
            ["Client", proj?.companyName || "—"],
            ["Date", report.date],
            ["Hours Worked", `${report.hours} hrs`],
            ["Manpower", `${report.manpowerCount || "—"} people`],
            ["Work Done", `${report.workQtyDone || "—"} ${proj?.unitType || ""}`],
          ].map(([k, v]) => (
        ...
        <W cls="fu1" style={{ overflow: "hidden" }}>
          <div style={{ padding: "12px 20px", borderBottom: `1px solid ${C.border}` }}>
            <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: C.light }}>
              {isOffSite ? "🌍 GPS Location (Off-Site)" : "📍 GPS Location"}
            </span>
          </div>
          <div style={{ position: "relative", height: 196, background: isOffSite ? "linear-gradient(145deg,#e9d5ff,#d8b4fe)" : "linear-gradient(145deg,#d4e6d4,#c0d8c0)" }}>
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8 }}>
              <div style={{ width: 26, height: 26, borderRadius: "50% 50% 50% 0", background: isOffSite ? "#7c3aed" : C.blue, transform: "rotate(-45deg)", boxShadow: isOffSite ? "0 4px 12px rgba(124,58,237,.5)" : `0 4px 12px rgba(37,99,235,.5)` }} />
          ...
          <div style={{ padding: "8px 20px", background: isOffSite ? "#f3e8ff" : "#f8faff" }}>
            <span style={{ fontSize: 11, color: C.light, fontWeight: 600 }}>
              {isOffSite ? "🌍 Off-site GPS captured · " : "Captured at submission · "}{report.date}
            </span>
          </div>
        </W>
      </div>
      ...
      {/* Report Image */}
      {(report.imageUrl || report.imageUploaded) && report.imageUrl && (
        <W cls="fu2" style={{ padding: 22, marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: C.light, marginBottom: 12 }}>📸 Attached Photo</div>
...
function LiveMap({ user, liveLocations = {}, onLogout, announcements = [], setEmployees, employees, profilePhotoPreview, setProfilePhotoPreview }) {
      ...
      const iconHtml = `
        ...
        ">
          <div style="
            ...
            overflow:hidden;
          ">
            ${emp.avatar && emp.avatar.match(/^https?:\/\//) ? `<img src="${emp.avatar}" style="width:100%;height:100%;object-fit:cover;" />` : emp.avatar}
          </div>
  ...
  return (
    <div>
      <TopBar title="Live Employee Tracking"
        ...
        user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />
            ...
            {selEmp && selLoc && (
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <Avt initials={selEmp.avatar} name={selEmp.name} size={34} />
                  <div>
              ...
              <W key={emp.id} style={{
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <Avt initials={emp.avatar} name={emp.name} size={34} />
                    <div style={{
...
// ─── Employees ─────────────────────────────────────────────────────────────────
function EmployeesPanel({ user, liveLocations = {}, onLogout, onEmployeeAdded, setEmployees }) {
  const [showAdd, setShowAdd] = useState(false);
  const [addForm, setAddForm] = useState({ name: "", email: "", department: "IT", role: "employee", password: "", avatar: null });
  const [addFormPhoto, setAddFormPhoto] = useState(null);
  const [addFormPhotoPreview, setAddFormPhotoPreview] = useState(null);
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      ...
      if (addFormPhoto) {
        ...
        try {
          avatarUrl = await uploadProfilePhoto(addFormPhoto, tempId);
        } catch (photoError) {
          console.error("Photo upload failed:", photoError);
        }
      }

      const emp = await createEmployee({ ...addForm, avatar: avatarUrl });
      ...
      setAddForm({ name: "", email: "", department: "IT", role: "employee", password: "", avatar: null });
  ...
  return (
    <div>
      <TopBar title="Employees" sub={`${MOCK_EMPLOYEES.length} active records`} user={user} onLogout={onLogout} setEmployees={setEmployees} />
      ...
      {showAdd && (
        <W cls="fu" style={{ padding: 24, marginBottom: 20 }}>
          ...
          {/* Profile Photo Upload for New Employee */}
          <div style={{ marginBottom: 14 }}>
            <Lbl>Profile Photo (Optional)</Lbl>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{
                width: 60, height: 60, borderRadius: "50%",
                background: addFormPhotoPreview ? `url(${addFormPhotoPreview}) center/cover` : C.bluePale,
                border: `2px solid ${addFormPhotoPreview ? 'transparent' : C.border}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                overflow: "hidden"
              }}>
                {!addFormPhotoPreview && <span style={{ fontSize: 24 }}>📷</span>}
              </div>
              <div style={{ flex: 1 }}>
                <input
                  type="file"
                  ...
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) {
                      setAddFormPhoto(file);
                      setAddFormPhotoPreview(URL.createObjectURL(file));
                    }
                  }}
                  style={{ display: "none" }}
                  id="add-employee-photo"
                />
                <label htmlFor="add-employee-photo">
                  <Btn v="secondary" as="span" style={{ cursor: "pointer", padding: "8px 16px" }}>
                    {addFormPhoto ? "📷 Change Photo" : "＋ Upload Photo"}
                  </Btn>
                </label>
                {addFormPhoto && (
                  <button
                    type="button"
                    onClick={() => { setAddFormPhoto(null); setAddFormPhotoPreview(null); }}
                    style={{ marginLeft: 8, background: "none", border: "none", color: "#dc2626", cursor: "pointer", fontWeight: 700, fontSize: 12 }}
                  >
                    ✕ Remove
                  </button>
                )}
          ...
          return (
            <W key={emp.id} cls={`fu${i > 3 ? 4 : i + 1}`} style={{ padding: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                <div style={{ position: "relative" }}>
                  <Avt initials={emp.avatar} name={emp.name} size={46} />
                  <div style={{
...
function AddProjectModal({ onClose, onAdd, projects, employees = [], prefill }) {
  const blankForm = {
  ...
  const [form, setForm] = useState(prefill ? {
    ...
    teamLeaderId: prefill.teamLeaderId || "",
    ...
    contactName: prefill.contactName || "", contactEmail: prefill.contactEmail || "", contactMobile: prefill.contactMobile || "",
  ...
  const save = () => {
    ...
    const proj = {
      ...
      assignedEmployees: form.assignedEmployees.includes(form.teamLeaderId)
        ? form.assignedEmployees
        : (form.teamLeaderId ? [...form.assignedEmployees, form.teamLeaderId] : form.assignedEmployees),
  ...
  const allEmployeesBank = (employees && employees.length > 0) ? employees : MOCK_EMPLOYEES;
  const deptEmps = form.department ? allEmployeesBank.filter(e => e.department === form.department) : allEmployeesBank;
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "20px 28px", maxHeight: "60vh", overflowY: "auto", minHeight: 280 }}>
          ...
          {step === 0 && (<>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
              <div>
                ...
                <FI value={form.contactName} onChange={e => setForm(p => ({ ...p, contactName: e.target.value }))} placeholder="e.g. Rahul Sharma" />
              </div>
              <div>
                <Lbl>Contact No</Lbl>
                <FI value={form.contactMobile} onChange={e => setForm(p => ({ ...p, contactMobile: e.target.value }))} placeholder="e.g. +91 9876543210" />
              </div>
            </div>

            <div style={{ marginBottom: 14 }}>
              <Lbl>Contact Email</Lbl>
              <FI value={form.contactEmail} onChange={e => setForm(p => ({ ...p, contactEmail: e.target.value }))} placeholder="e.g. rahul@example.com" />
            </div>
          </>)}
          ...
          {step === 2 && (<>
            ...
            <div style={{ marginBottom: 20 }}>
              <Lbl>Team Leader *</Lbl>
              <FS value={form.teamLeaderId} onChange={e => {
                const tlId = e.target.value;
                setForm(p => ({
                  ...p,
                  teamLeaderId: tlId,
                  // Auto-add TL to assigned employees if they change it
                  assignedEmployees: tlId && !p.assignedEmployees.includes(tlId)
                    ? [...p.assignedEmployees, tlId]
                    : p.assignedEmployees
                }));
              }}>
                <option value="">Select a Team Leader…</option>
                {allEmployeesBank.filter(e => e.role === "tl" || e.role === "admin").map(emp => (
                  <option key={emp.id} value={emp.id}>{emp.name} ({emp.department})</option>
                ))}
              </FS>
              <div style={{ fontSize: 10, color: C.light, marginTop: 4 }}>Note: Selecting a team leader will automatically add them to the team below.</div>
            </div>
                ...
                {deptEmps.map(emp => {
                  ...
                  return (
                    <div key={emp.id} onClick={() => toggleEmp(emp.id)} style={{
                      ...
                      <Avt initials={emp.avatar} name={emp.name} size={32} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
                          {emp.name}
                          {emp.id === form.teamLeaderId && <span style={{ marginLeft: 6, fontSize: 10, color: C.blue, fontWeight: 800 }}>[Leader]</span>}
                        </div>
...
function ProjectDetail({ project, reports, projectItems = [], employees = [], onClose, onStatusChange, onEdit }) {
  const assignedEmps = (project.assignedEmployees || []).map(id => (employees.length ? employees : MOCK_EMPLOYEES).find(e => e.id === id)).filter(Boolean);
  const projReports = reports.filter(r => r.projectId === project.id);
  const totalHours = projReports.reduce((s, r) => s + Number(r.hours), 0);
  const [tab, setTab] = useState("overview");
  const [showTlChange, setShowTlChange] = useState(false);
  const [selectedTlId, setSelectedTlId] = useState("");
  const [changingTl, setChangingTl] = useState(false);
  const [tlChanged, setTlChanged] = useState(false);

  // Get ALL team leaders from the bank of all employees
  const allEmployeesBank = employees.length ? employees : MOCK_EMPLOYEES;
  const tlEmployees = allEmployeesBank.filter(e => e.role === "tl") || [];
  const currentTl = allEmployeesBank.find(e => e.id === project.teamLeaderId) || assignedEmps.find(e => e.role === "tl") || assignedEmps[0];

  // Handle team leader change
  const handleTeamLeaderChange = async () => {
    if (!selectedTlId || selectedTlId === currentTl?.id) return;
    setChangingTl(true);
    try {
      await updateTeamLeader(project.id, currentTl?.id, selectedTlId);
      setTlChanged(true);
      setTimeout(() => {
        setTlChanged(false);
        setShowTlChange(false);
        // Refresh the page to show updated data
        window.location.reload();
      }, 1500);
    } catch (error) {
      console.error("Failed to change team leader:", error);
      alert("Failed to change team leader. Please try again.");
    } finally {
      setChangingTl(false);
    }
  };
          ...
          {tab === "overview" && (
            <div>
              ...
              {(project.poNumber || project.companyName) && (() => {
                // Find TL assigned to this project - use teamLeaderId first
                const tlEmp = allEmployeesBank.find(e => e.id === project.teamLeaderId) ||
                  (project.assignedEmployees || [])
                    .map(id => allEmployeesBank.find(e => e.id === id))
                    .filter(Boolean)
                    .find(e => e.role === "tl");
                return (
                  <div style={{ background: "#f0f9ff", border: `1px solid ${C.blueMid}`, borderRadius: 14, padding: "14px 18px", marginBottom: 16 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      {[
                        ...
                        ["Contact Mobile", project.contactMobile],
                      ].filter(([, v]) => v).map(([k, v]) => (
                        <div key={k}>
                    ...
                    <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${C.blueMid}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: C.light }}>👷 Team Leader</div>
                        {tlEmployees.length > 0 && (
                          <button
                            onClick={() => setShowTlChange(!showTlChange)}
                            style={{
                        ...
                        <div style={{
                          ...
                          <select
                            ...
                            style={{
                              ...
                              border: `1px solid ${C.border}`, fontSize: 12, marginBottom: 8
                            }}
                          >
                          ...
                          <div style={{ display: "flex", gap: 8 }}>
                            <button
                              onClick={handleTeamLeaderChange}
                              disabled={!selectedTlId || changingTl}
                              style={{
                                flex: 1, padding: "8px 12px", borderRadius: 8, border: "none",
                                background: changingTl ? "#ccc" : C.blue, color: "#fff",
                                fontSize: 11, fontWeight: 700, cursor: changingTl ? "not-allowed" : "pointer"
                              }}
                            >
                              {changingTl ? "⏳ Changing..." : "✅ Confirm Change"}
                            </button>
                            <button
                              onClick={() => setShowTlChange(false)}
                              style={{
                                padding: "8px 12px", borderRadius: 8, border: `1px solid ${C.border}`,
                                background: "#fff", color: C.text, fontSize: 11, fontWeight: 700, cursor: "pointer"
                              }}
                            >
                              Cancel
                            </button>
                          ...
                          {tlChanged && (
                            <div style={{
                              ...
                              background: "#f0fdf4", color: "#059669", fontSize: 11, fontWeight: 700,
                              textAlign: "center"
                            }}>
                              ✅ Team leader changed! All reports transferred to new leader.
                            </div>
                          )}
                        </div>
                      )}
              ...
              {project.totalWorkQty && (() => {
                const completedQty = projReports.reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0);
                  ...
                  {assignedEmps.map(emp => {
                    ...
                    return (
                      <W key={emp.id} style={{ padding: "16px 18px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <Avt initials={emp.avatar} name={emp.name} size={40} />
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>{emp.name}</div>
                            ...
                            <td style={{ padding: "10px 14px" }}>
                              <div style={{ display: "flex", gap: 4 }}>
                                {[...d.workers].map(id => {
                                  ...
                                  return e ? <Avt key={id} initials={e.avatar} name={e.name} size={22} /> : null;
                                })}
                              </div>
                            </td>
                          </tr>
                          ...
                          <tr key={r.id} style={{ borderTop: `1px solid #f0f6ff` }}>
                            ...
                            <td style={{ padding: "10px 12px" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                <Avt initials={emp?.avatar} name={emp?.name} size={22} />
...
function TLAnnouncementPanel({ user, announcements, onSend, onLogout, setEmployees }) {
  ...
  return (
    <div>
      <TopBar title="Announcements" sub={`Send messages to your team`} user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />
                ...
                return (
                  <div key={emp.id} onClick={() => toggleRec(emp.id)}
                    ...
                    <div style={{ marginRight: 8, display: "flex", alignItems: "center" }}>
                      <Avt initials={emp.avatar} name={emp.name} size={24} />
                    </div>
...
function EmployeeAnnouncementsPanel({ user, announcements, onLogout, setEmployees }) {
  const PRIORITY_CFG = {
    normal: { label: "Normal", color: "#2563eb", bg: "#eff6ff", icon: "📋" },
    urgent: { label: "🚨 Urgent", color: "#dc2626", bg: "#fef2f2", icon: "🚨" },
    info: { label: "ℹ Info", color: "#059669", bg: "#f0fdf4", icon: "ℹ️" },
  ...
  return (
    <div>
      <TopBar title="Announcements" sub={`Messages from your team leader`} user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />
              ...
              <div key={a.id}
                ...
                <div style={{ padding: "16px 20px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {(() => { const emp = MOCK_EMPLOYEES.find(e => e.id === a.fromId); return <Avt initials={emp?.avatar} name={emp?.name || a.from} size={36} />; })()}
                      <div>
...
function TLTeamManageModal({ project, user, employees = [], onClose, onSave }) {
  // All employees across the whole company
  const allEmployeesBank = employees.length ? employees : MOCK_EMPLOYEES;
  const allEmps = allEmployeesBank.filter(e => e.role === "employee");
  ...
  const assignedEmps = assigned.map(id => allEmployeesBank.find(e => e.id === id)).filter(Boolean);
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {visibleUnassigned.map(emp => (
                  <div key={emp.id}
                    ...
                    onMouseOut={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.background = "#f8faff"; }}>
                    <div style={{ marginRight: 10, display: "flex", alignItems: "center", opacity: 0.8 }}>
                      <Avt initials={emp.avatar} name={emp.name} size={40} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{
...
// ─── TL Projects Panel ──────────────────────────────────────────────────────────
function TLProjectsPanel({ user, projects, reports, projectItems = [], employees = [], onEditProject, onLogout, setEmployees }) {
  const allProjects = projects || MOCK_PROJECTS;
  const allReports = reports || MOCK_REPORTS;
  ...
  const ProjectCard = ({ p }) => {
    ...
    const rpts = allReports.filter(r => r.projectId === p.id);
    const isCompleted = p.status === "completed" || p.status === "done";
    const rawCompletedQty = rpts.reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0);
    ...
    const assignedEmps = (p.assignedEmployees || []).map(id => allEmployeesBank.find(e => e.id === id)).filter(Boolean);

    // Unique workers who submitted reports
    const workerIds = [...new Set(rpts.map(r => r.employeeId))];
    const workers = workerIds.map(id => allEmployeesBank.find(e => e.id === id)).filter(Boolean);
        ...
        <div style={{ padding: "18px 20px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div style={{ flex: 1, paddingRight: 12 }}>
              ...
              {(() => {
                const tl = allEmployeesBank.find(e => e.id === p.teamLeaderId);
                return tl && tl.id !== user.id ? <div style={{ fontSize: 11, color: C.blue, fontWeight: 800, marginBottom: 4 }}>👷 Leader: {tl.name}</div> : null;
              })()}
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[
                  ...
                  ["Contact Mobile", p.contactMobile],
                ].filter(([, v]) => v).map(([k, v]) => (
                  <div key={k}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ display: "flex" }}>
                {assignedEmps.slice(0, 5).map((emp, idx) => (
                  <div key={emp.id} style={{ marginLeft: idx > 0 ? -6 : 0, zIndex: 5 - idx }}>
                    <Avt initials={emp.avatar} name={emp.name} size={26} />
                  </div>
                ))}
  ...
  return (
    <div>
      <TopBar
        ...
        user={user} onLogout={onLogout} setEmployees={setEmployees} />
...
// ─── Projects Panel ─────────────────────────────────────────────────────────────
function ProjectsPanel({ user, projects, reports, projectItems = [], employees = [], onAddProject, onStatusChange, onEditProject, onLogout, setEmployees }) {
  const [showAdd, setShowAdd] = useState(false);
  const [selProj, setSelProj] = useState(null);
  const [editProj, setEditProj] = useState(null);
  ...
  const companies = [...new Set(projects.map(p => p.companyName).filter(Boolean))];

  // Get unique months from projects (format: YYYY-MM)
  const months = [...new Set(projects.map(p => {
    if (p.startDate) {
  ...
  const filtered = projects.filter(p => {
    ...
    // Month filter
    if (monthFilter !== "all") {
      const projectMonth = p.startDate ?
        `${new Date(p.startDate).getFullYear()}-${String(new Date(p.startDate).getMonth() + 1).padStart(2, '0')}` : null;
      if (projectMonth !== monthFilter) return false;
    }
    return true;
  });
  ...
  useEffect(() => {
    projects.forEach(p => {
      if (p.status === "active") {
        ...
        const completedQty = rpts.reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0);
        const totalQty = p.totalWorkQty || 0;
  ...
  return (
    <div>
      <TopBar title="Projects" sub={`${filtered.length} projects · ${activeCount} active · ${completedCount} completed`} user={user} onLogout={onLogout} setEmployees={setEmployees} />

      {/* Stats strip */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 20 }} className="fu">
        {[
          { l: "Total Projects", v: filtered.length, icon: "🚀", grad: `linear-gradient(135deg,${C.blue},${C.blueL})` },
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          {companies.length > 0 && (
            <select
              ...
              style={{
                ...
                fontSize: 12, fontWeight: 700, background: companyFilter === "all" ? C.bluePale : C.blue,
                color: companyFilter === "all" ? C.blue : "#fff",
                boxShadow: companyFilter === "all" ? "none" : `0 3px 10px rgba(37,99,235,.28)`
              }}
            >
          ...
          {months.length > 0 && (
            <select
              ...
              style={{
                padding: "8px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: "inherit",
                fontSize: 12, fontWeight: 700, background: monthFilter === "all" ? C.bluePale : C.blue,
                color: monthFilter === "all" ? C.blue : "#fff",
                boxShadow: monthFilter === "all" ? "none" : `0 3px 10px rgba(37,99,235,.28)`
              }}
            >
          ...
          {(companyFilter !== "all" || monthFilter !== "all") && (
            <button onClick={() => { setCompanyFilter("all"); setMonthFilter("all"); }} style={{
              padding: "8px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: "inherit",
            ...
            <W key={p.id} cls={`fu${i > 3 ? 4 : i % 4 + 1}`} style={{ padding: 0, cursor: "pointer", transition: "transform .15s,box-shadow .15s" }}
              ...
              <div style={{ padding: "18px 20px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 3, marginBottom: 10 }}>
                  {p.companyName && <div style={{ fontSize: 11, color: C.muted, fontWeight: 700 }}>🏢 {p.companyName}</div>}
                  {(() => {
                    const allEmpsBank = (employees && employees.length > 0) ? employees : MOCK_EMPLOYEES;
                    const tl = allEmpsBank.find(e => e.id === p.teamLeaderId);
                    return tl ? <div style={{ fontSize: 11, color: C.blue, fontWeight: 800 }}>👷 Leader: {tl.name}</div> : null;
                  })()}
                </div>

                {/* Description & Contact */}
                {p.description && (
                  <p style={{ fontSize: 12, color: C.muted, margin: "0 0 8px", lineHeight: 1.55, fontWeight: 600 }}>
                    {p.description.slice(0, 70)}{p.description.length > 70 ? "…" : ""}
                  </p>
                )}
                {p.contactName && (
                  <div style={{ fontSize: 10, color: C.light, fontWeight: 700, marginBottom: 10, background: "#f8faff", padding: "4px 8px", borderRadius: 6 }}>
                    📞 {p.contactName} {p.contactMobile ? `(${p.contactMobile})` : ""}
                  </div>
                )}
...
// ─── Departments ───────────────────────────────────────────────────────────────
function DepartmentsPanel({ user, onLogout, setEmployees }) {
  ...
  return (
    <div>
      <TopBar title="Departments" sub={`${DEPARTMENTS.length} operational units`} user={user} onLogout={onLogout} setEmployees={setEmployees} />
...
function ProgressDetailModal({ project, reports, projectItems, onClose }) {
  ...
  const { completedQty, totalQty, remaining, progress } = calculateProjectProgress(project, reports || MOCK_REPORTS, projectItems);
  const isCompleted = project.status === "completed" || project.status === "done";
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          {(projectItems || []).filter(i => i.projectId === project.id).length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {(() => {
                  const itemIds = new Set((projectItems || []).filter(it => it.projectId === project.id).map(it => it.id));
                  const unlinkedQty = projReports.filter(r => !itemIds.has(r.projectItemId)).reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0);
                  const itemsOnProj = (projectItems || []).filter(i => i.projectId === project.id);

                  return (
                    <>
                      {unlinkedQty > 0 && (
                        <div style={{ background: "#f8faff", border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", opacity: 0.9 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                            <span style={{ fontSize: 12, fontWeight: 800, color: C.text }}>📦 General Project Scope</span>
                            <span style={{ fontSize: 11, fontWeight: 900, color: C.blue }}>Direct Progress</span>
                          </div>
                          <div style={{ height: 6, background: "#e2e8f0", borderRadius: 6, overflow: "hidden", marginBottom: 6 }}>
                            <div style={{ height: "100%", width: "100%", borderRadius: 6, background: "#60a5fa", opacity: 0.5 }} />
                          </div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, fontWeight: 700 }}>
                            <span style={{ color: C.muted }}>Main Unit: {project.unitType}</span>
                            <span style={{ color: "#059669" }}>Done: {unlinkedQty} {project.unitType}</span>
                          </div>
                          {remaining > 0 && (
                            <div style={{ fontSize: 10, color: C.blue, fontWeight: 700, padding: "6px 8px", background: C.blueSoft, borderRadius: 8, marginTop: 10, border: "1px solid rgba(37,99,235,.15)" }}>
                              💡 To log remaining {remaining} {project.unitType}, use the <b>Submit Report</b> form and select <b>General Project Scope</b>.
                            </div>
                          )}
                        </div>
                      )}
                      {itemsOnProj.map(item => {
                    ...
                    return (
                      <tr key={r.id} style={{
                        ...
                        <td style={{ padding: "9px 10px" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                            <Avt initials={emp?.avatar} name={emp?.name} size={20} />
...
// ─── Submit Report ─────────────────────────────────────────────────────────────
function ReportSubmission({ employee, user, projects, reports, projectItems, onSubmit, onLogout, isTL = false, announcements = [], setEmployees }) {
  const allProjects = projects || MOCK_PROJECTS;
  const allReports = reports || MOCK_REPORTS;
  const allProjectItems = projectItems || [];

  // Off-site work report toggle
  const [isOffSite, setIsOffSite] = useState(false);

  // Off-site work types
  const OFF_SITE_WORK_TYPES = [
    "Material Purchase",
    "Site Survey",
    "Client Meeting",
    "Support Work",
  ...
  const completedQty = selItem
    ...
    : selProj
      ? allReports.filter(r => r.projectId === selProj.id && (!r.projectItemId || allProjectItems.find(it => it.id === r.projectItemId)?.unit === selProj.unitType)).reduce((s, r) => s + Math.round(Number(r.workQtyDone || 0)), 0)
      : 0;
  ...
  const validate = () => {
    ...
    if (!isOffSite) {
      ...
      if (projBOQItems.length > 0 && !form.projectItemId && (!selProj || !selProj.totalWorkQty)) {
        e.projectItemId = "Please select a specific Work Item from the project scope.";
      }
      ...
      // Off-site report validation
      if (!form.offSiteWorkType) e.offSiteWorkType = "Select a work type.";
      if (!form.offSiteLocation || form.offSiteLocation.trim().length < 3) e.offSiteLocation = "Enter the location (min 3 characters).";
      if (!form.offSitePurpose || form.offSitePurpose.trim().length < 10) e.offSitePurpose = "Enter the purpose (min 10 characters).";
    }
  ...
  const submit = async () => {
    ...
    const newReport = {
      ...
      projectId: isOffSite ? null : form.project,
      date: new Date().toISOString().slice(0, 10),
      hours: Number(form.hours),
      manpowerCount: Number(form.manpowerCount),
      workQtyDone: isOffSite ? 0 : Math.round(Number(form.workQtyDone)),
      projectItemId: isOffSite ? null : (form.projectItemId || null),
      ...
      // Off-site fields
      isOffSite: isOffSite,
      offSiteWorkType: isOffSite ? form.offSiteWorkType : null,
      offSiteLocation: isOffSite ? form.offSiteLocation.trim() : null,
      offSitePurpose: isOffSite ? form.offSitePurpose.trim() : null,
    };
  ...
  if (submitted) {
    ...
    // Build notification message based on report type
    let msgText;
    if (rep.isOffSite) {
      msgText =
        `🌍 *Off-Site Work Report Submitted*
👷 *Employee:* ${employee.name}
📅 *Date:* ${today}
⏱️ *Hours:* ${rep.hours}h
👥 *Manpower:* ${rep.manpowerCount} pax
🔖 *Work Type:* ${rep.offSiteWorkType}
📍 *Location:* ${rep.offSiteLocation}
🎯 *Purpose:* ${rep.offSitePurpose}
...
${rep.issuesFaced?.length ? "⚠️ *Issues:* " + rep.issuesFaced.join(", ") : "✅ No issues reported"}`;
    ...
    const emailSubject = encodeURIComponent(`${rep.isOffSite ? "Off-Site" : "Work"} Report – ${employee.name} – ${today}`);
    ...
    return (
      <div>
        <TopBar title={isTL ? "Submit Team Report" : "Submit Report"} user={user} onLogout={onLogout} setEmployees={setEmployees} />
        <div style={{ maxWidth: 540, margin: "30px auto" }}>
          {/* Success card */}
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            <div style={{
              width: 72, height: 72, borderRadius: "50%", background: rep.isOffSite ? "linear-gradient(135deg,#7c3aed,#a855f7)" : "linear-gradient(135deg,#059669,#10b981)",
              display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px",
              fontSize: 32, color: "#fff", boxShadow: rep.isOffSite ? "0 8px 24px rgba(124,58,237,.35)" : "0 8px 24px rgba(5,150,105,.35)"
            }}>✓</div>
            <h3 style={{ fontSize: 22, fontWeight: 900, color: C.text, margin: "0 0 6px" }}>
              {rep.isOffSite ? "Off-Site Report Submitted!" : "Report Submitted!"}
            </h3>
            <p style={{ fontSize: 13, color: C.muted, margin: "0", fontWeight: 600 }}>
              {rep.isOffSite
                ? `Your off-site work (${rep.offSiteWorkType}) has been recorded.`
          ...
          <div style={{
            ...
            {/* Off-site location info */}
            {rep.isOffSite && (
              <div style={{ marginTop: 12, padding: "10px 12px", background: "#f3e8ff", borderRadius: 10, border: "1px solid #d8b4fe" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#7c3aed", marginBottom: 4 }}>📍 Location</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{rep.offSiteLocation}</div>
              </div>
            )}
          </div>
          ...
          <Btn v="primary" style={{ width: "100%", justifyContent: "center" }}
            onClick={() => { setSubmitted(false); setIsOffSite(false); setImageFile(null); setImagePreview(null); setForm({ project: "", projectItemId: "", manpowerCount: "", workQtyDone: "", workDetails: "", tasksCompleted: "", workInProgress: "", issues: "", planNextDay: "", remarks: "", hours: "", hasImage: false, offSiteWorkType: "", offSiteLocation: "", offSitePurpose: "" }); }}>
  ...
  return (
    <div>
      <TopBar title={isTL ? "Submit Team Report" : "Submit Daily Report"}
        sub={new Date().toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" })} user={user} onLogout={onLogout} setEmployees={setEmployees} />
      <div style={{ maxWidth: 660 }}>
        <W cls="fu" style={{ padding: 28 }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {/* Regular Report Card */}
              <div onClick={() => setIsOffSite(false)} style={{
                padding: "16px 18px", borderRadius: 14, cursor: "pointer",
                border: `2px solid ${!isOffSite ? C.blue : C.border}`,
                background: !isOffSite ? C.bluePale : "#f8faff",
                transition: "all .2s"
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 22 }}>🏗</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: !isOffSite ? C.blue : C.text }}>Regular Site Report</div>
                    <div style={{ fontSize: 10, color: C.light, fontWeight: 600 }}>Link to assigned project</div>
                  </div>
                </div>
                <div style={{ fontSize: 10, color: C.muted, fontWeight: 600 }}>
                  • Project-linked work<br />
                  • BOQ qty tracking<br />
                  • Progress updates
                </div>
              </div>
              {/* Off-Site Report Card */}
              <div onClick={() => setIsOffSite(true)} style={{
                padding: "16px 18px", borderRadius: 14, cursor: "pointer",
                border: `2px solid ${isOffSite ? "#7c3aed" : C.border}`,
                background: isOffSite ? "#f3e8ff" : "#f8faff",
                transition: "all .2s"
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 22 }}>🌍</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: isOffSite ? "#7c3aed" : C.text }}>Off-Site / Ad-hoc Work</div>
                    <div style={{ fontSize: 10, color: C.light, fontWeight: 600 }}>No project link needed</div>
                  </div>
                </div>
                <div style={{ fontSize: 10, color: C.muted, fontWeight: 600 }}>
                  • Material purchase<br />
                  • Site survey / meeting<br />
                  • No BOQ tracking
                </div>
              </div>
            </div>
          ...
          {/* ── OFF-SITE FIELDS (only show when isOffSite is true) ── */}
          {isOffSite && (
            <div style={{ marginBottom: 24, padding: "20px 18px", background: "#f3e8ff", borderRadius: 16, border: "2px solid #d8b4fe" }}>
              <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", color: "#7c3aed", marginBottom: 16 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 6 }}>
                  {OFF_SITE_WORK_TYPES.map(type => (
                    <div key={type} onClick={() => setForm(p => ({ ...p, offSiteWorkType: type }))} style={{
                      padding: "8px 14px", borderRadius: 20, cursor: "pointer", fontSize: 11, fontWeight: 700,
                      background: form.offSiteWorkType === type ? "#7c3aed" : "#fff",
                      color: form.offSiteWorkType === type ? "#fff" : "#7c3aed",
                      border: `1.5px solid ${form.offSiteWorkType === type ? "#7c3aed" : "#d8b4fe"}`,
                      transition: "all .15s"
                    }}>
                      {type}
                    </div>
                ...
                <ErrMsg k="offSiteWorkType" />
              </div>

              {/* Location */}
              <div style={{ marginBottom: 16 }}>
                <Lbl>Site / Location Name *</Lbl>
                <FI value={form.offSiteLocation} onChange={e => setForm(p => ({ ...p, offSiteLocation: e.target.value }))}
                  placeholder="e.g. Supplier warehouse, Andheri West · Client office, BKC" />
                <ErrMsg k="offSiteLocation" />
              </div>

              {/* Purpose */}
              <div>
                <Lbl>Purpose / Objective *</Lbl>
                <FI value={form.offSitePurpose} onChange={e => setForm(p => ({ ...p, offSitePurpose: e.target.value }))}
                  placeholder="e.g. Purchased 500m Cat6 cable for PO-2025-014" rows={2} />
                <ErrMsg k="offSitePurpose" />
              </div>
            </div>
          )}

          {/* ── SECTION: PO / Project (hidden when off-site) ── */}
          {!isOffSite && (
            <>
          ...
          {/* Item selector - shows when project has BOQ items (hidden for off-site) */}
          {!isOffSite && selProj && projBOQItems.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <Lbl>Work Item *</Lbl>
              <FS value={form.projectItemId} onChange={e => setForm(p => ({ ...p, projectItemId: e.target.value }))}>
                <option value="">{projBOQItems.length > 0 ? "General Work / Main Project Scope" : "Direct Project Progress"}</option>
            ...
            <div style={{ background: "#f0f9ff", border: `1px solid ${C.blueMid}`, borderRadius: 14, padding: "14px 18px", marginBottom: 20 }}>
              ...
              <div
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, alignItems: "center" }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: C.muted }}>{selItem ? "Selected Item Progress" : "Overall Project Progress"}</span>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5, fontSize: 10, fontWeight: 700 }}>
                  <span style={{ color: C.muted }}>Total: <strong style={{ color: C.text }}>{totalQty} {selItem ? selItem.unit : selProj.unitType}</strong></span>
                  <span style={{ color: "#059669" }}>Done: <strong>{completedQty} {selItem ? selItem.unit : selProj.unitType}</strong></span>
                  <span style={{ color: remaining <= 0 ? "#059669" : "#dc2626", fontWeight: 800 }}>Remaining: <strong>{Math.max(0, remaining)} {selItem ? selItem.unit : selProj.unitType}</strong></span>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            ...
            <div>
              {!isOffSite ? (
                ...
                <>
                  <Lbl>Work Done <span style={{ fontSize: 10, color: C.light, fontWeight: 600 }}>(optional for off-site)</span></Lbl>
                  <FI type="number" min="0" step="1" value={form.workQtyDone}
                    onChange={e => setForm(p => ({ ...p, workQtyDone: e.target.value.replace(/\./g, '') }))}
                    placeholder="0" />
                </>
              )}
...
// ─── Employee History ──────────────────────────────────────────────────────────
function EmployeeHistory({ employee, user, reports, onLogout, announcements = [], setEmployees }) {
  const allReports = reports || MOCK_REPORTS;
  const rpts = allReports.filter(r => r.employeeId === employee.id);
  return (
    <div>
      <TopBar title="My Reports" sub={`${rpts.length} submissions`} user={user} onLogout={onLogout} announcements={announcements} setEmployees={setEmployees} />
...
export default function App() {
  ...
  const [profilePhotoPreview, setProfilePhotoPreview] = useState(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('userAvatarUrl') || null;
    }
    return null;
  });

  // Sync profile photo preview if user avatar changes (e.g. after login/re-fetch)
  useEffect(() => {
    if (authUser?.avatar && !profilePhotoPreview) {
      setProfilePhotoPreview(authUser.avatar);
    }
  }, [authUser?.avatar]);
    ...
    const restoreSession = async () => {
      try {
        ...
        if (sessionUser && isMounted) {
          // Session exists, set role and user
          setAuthUser(sessionUser);
          setRole(sessionUser.role);
          setSection(sessionUser.role === "admin" ? "overview" : (sessionUser.role === "tl" ? "tl-submit" : "submit"));

          // Load fresh data from database after restoring session
          await loadData({ role: sessionUser.role });
        }
      } catch (err) {
  ...
  // Load data from database with optimized fetching
  // Fetches only what's needed based on role, with pagination and caching
  const loadData = async (options = {}) => {
    const { role: userRole = role, forceRefresh = false } = options;

    try {
      // Phase 1: Critical data for all roles (parallel fetch with caching)
      const [emps, projs] = await Promise.all([
        fetchEmployees({ forceRefresh }),
        fetchProjects({ forceRefresh }),
      ]);

      // Update global mocks immediately
      if (emps.length > 0) { MOCK_EMPLOYEES.length = 0; emps.forEach(e => MOCK_EMPLOYEES.push(e)); }
      if (emps.length > 0) setEmployees(emps);
      setProjects(projs);

      // Phase 2: Role-specific data (deferred based on current view)
      // Admin needs reports and announcements (paginated)
      // TL needs reports for their projects
      // Employee needs their own reports and announcements

      const isAdmin = userRole === "admin";
      const isTL = userRole === "tl";

      // Fetch reports with pagination (50 initially) for admin/TL
      if (isAdmin || isTL) {
        console.log("DEBUG: YES - fetching reports for admin/tl, role:", userRole, "isAdmin:", isAdmin, "isTL:", isTL);
        const reportsResult = await fetchReports({
          limit: 50,
          offset: 0,
          forceRefresh
        });
        console.log("DEBUG: reports fetch complete, result:", reportsResult);
        const reportsArray = Array.isArray(reportsResult) ? reportsResult : (reportsResult?.reports || []);
        console.log("DEBUG: setting reports state, count:", reportsArray.length);
        setReports(reportsArray);
      } else {
        console.log("DEBUG: SKIPPING reports fetch - userRole:", userRole, "isAdmin:", isAdmin, "isTL:", isTL);
      }

      // Fetch announcements with pagination (20 initially)
      const annResult = await fetchAnnouncements({
        limit: 20,
        offset: 0,
        forceRefresh
      });
      const annsArray = Array.isArray(annResult) ? annResult : (annResult?.announcements || []);
      setAnnouncements(annsArray);

      // Fetch project items on initial load to ensure persistence across refresh/logout
      const pItems = await fetchProjectItems({ forceRefresh: true });
  ...
  const loadMoreReports = async () => {
    try {
      ...
      const newReports = Array.isArray(result) ? result : (result?.reports || []);
  ...
  // Load more announcements for pagination
  const loadMoreAnnouncements = async () => {
    try {
      ...
      const newAnnouncements = Array.isArray(result) ? result : (result?.announcements || []);
  ...
  // Lazy load project items when viewing a project
  const loadProjectItems = async (projectId) => {
    try {
      const items = await fetchProjectItems({ projectId, forceRefresh: true });
      setProjectItems(prev => {
        // Merge with existing items, avoiding duplicates
        const existingIds = new Set(prev.map(i => i.id));
        const newItems = items.filter(i => !existingIds.has(i.id));
        console.log("loadProjectItems - projectId:", projectId, "items found:", items.length, "new items:", newItems.length);
  ...
  useEffect(() => {
    if (selProj?.id) {
      // Check if we already have items for this project
      const hasItems = projectItems.some(i => i.projectId === selProj.id);
      if (!hasItems) {
        console.log("Auto-loading project items for:", selProj.id);
        loadProjectItems(selProj.id);
      }
    }
  }, [selProj?.id]);
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'projects' }, (payload) => {
        const mapProj = (p, current = {}) => ({
          ...
          totalWorkQty: p.total_work_qty !== undefined ? Number(p.total_work_qty) : (current.totalWorkQty || 0),
          unitType: p.unit_type || current.unitType, workType: p.work_type || current.workType,
          department: p.department || current.department, status: p.status || current.status,
          startDate: p.start_date || current.startDate, endDate: p.end_date || current.endDate,
          description: p.description || current.description,
          assignedEmployees: p.assigned_employees || current.assignedEmployees || [],
          teamLeaderId: p.team_leader_id || current.teamLeaderId,
          lastUpdatedAt: p.last_updated_at || current.lastUpdatedAt,
          lastUpdateType: p.last_update_type || current.lastUpdateType,
          contactName: p.contact_person_name !== undefined ? p.contact_person_name : (current.contactName || ""),
          contactEmail: p.contact_person_email !== undefined ? p.contact_person_email : (current.contactEmail || ""),
          contactMobile: p.contact_person_mobile !== undefined ? p.contact_person_mobile : (current.contactMobile || ""),
        });
        if (payload.eventType === 'INSERT') {
          ...
          setProjects(prev => prev.map(p => {
            if (p.id === payload.new.id) return mapProj(payload.new, p);
            return p;
          }));
        } else if (payload.eventType === 'DELETE') {
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'project_items' }, (payload) => {
        const mapProjectItem = (item) => ({
          id: item.id,
          projectId: item.project_id,
          description: item.item_name,
          quantity: Number(item.supplied_qty) || 0,
          usedQty: Number(item.used_qty || 0),
          unit: item.unit,
        ...
        if (payload.eventType === 'INSERT') {
          const mapped = mapProjectItem(payload.new);
          setProjectItems(prev => prev.some(x => x.id === mapped.id) ? prev : [...prev, mapped]);
        } else if (payload.eventType === 'UPDATE') {
          const mapped = mapProjectItem(payload.new);
          setProjectItems(prev => prev.map(x => x.id === mapped.id ? mapped : x));
        } else if (payload.eventType === 'DELETE') {
          setProjectItems(prev => prev.filter(x => x.id !== payload.old.id));
        }
      })
      .subscribe();
  ...
  // Memoize user to prevent unnecessary re-renders - derive stable user from authUser and role
  const user = useMemo(() => {
    if (authUser) return { ...authUser, avatar: profilePhotoPreview || authUser.avatar };
    if (role === "admin") {
      return { id: "admin1", name: "Sarah Mitchell", email: "admin@corp.com", department: "Administration", avatar: profilePhotoPreview || "SM" };
    }
    // Find the logged-in employee from the employees list by matching ID
    // First check if there's an authUser id to match, otherwise use first employee
    const targetId = authUser?.id || employees?.[0]?.id;
    const matchedEmployee = targetId ? employees?.find(e => e.id === targetId) : employees?.[0];
    return matchedEmployee ? {
      id: matchedEmployee.id,
      name: matchedEmployee.name,
      email: matchedEmployee.email,
      department: matchedEmployee.department,
      avatar: profilePhotoPreview || matchedEmployee.avatar
    } : { id: "unknown", name: "User", email: "", department: "", avatar: profilePhotoPreview || "U" };
  }, [authUser, role, employees, profilePhotoPreview]); // Included profilePhotoPreview as dependency

  const login = async (r, userData) => {
    console.log("DEBUG: login called with role:", r);
    setRole(r);
    ...
    console.log("DEBUG: login calling loadData()");
    await loadData({ role: r });
  };
  const logout = async () => {
  ...
  const handleAddProject = async (proj) => {
    const enriched = {
      ...
      contactMobile: proj.contactMobile,
    };
    console.log("Adding Project - Payload:", enriched);
  ...
  const handleEditProject = async (updated) => {
    ...
    console.log("Editing Project - Payload:", enriched);
  ...
  const content = () => {
    if (selReport) return <ReportDetail report={selReport} onBack={() => setSelReport(null)} user={user} onLogout={logout} announcements={announcements} employees={employees} projects={projects} setEmployees={setEmployees} profilePhotoPreview={profilePhotoPreview} setProfilePhotoPreview={setProfilePhotoPreview} />;
    if (role === "admin") {
      switch (section) {
        case "overview": return <AdminOverview onNavigate={nav} user={user} projects={projects} reports={reports} projectItems={projectItems} employees={employees} onStatusChange={handleStatusChange} onEditProject={handleEditProject} onLogout={logout} announcements={announcements} setEmployees={setEmployees} profilePhotoPreview={profilePhotoPreview} setProfilePhotoPreview={setProfilePhotoPreview} />;
        case "reports": return <ReportsList onSelect={setSelReport} user={user} onLogout={logout} reports={reports} employees={employees} projects={projects} announcements={announcements} setEmployees={setEmployees} profilePhotoPreview={profilePhotoPreview} setProfilePhotoPreview={setProfilePhotoPreview} />;
        case "map": return <LiveMap user={user} liveLocations={liveLocations} onLogout={logout} announcements={announcements} setEmployees={setEmployees} employees={employees} profilePhotoPreview={profilePhotoPreview} setProfilePhotoPreview={setProfilePhotoPreview} />;
        case "employees": return <EmployeesPanel user={user} liveLocations={liveLocations} onLogout={logout} onEmployeeAdded={(emp) => setEmployees(prev => [...prev, emp])} announcements={announcements} setEmployees={setEmployees} />;
        case "projects": return <ProjectsPanel user={user} projects={projects} reports={reports} projectItems={projectItems} employees={employees}
          onAddProject={handleAddProject} onStatusChange={handleStatusChange} onEditProject={handleEditProject} onLogout={logout} announcements={announcements} setEmployees={setEmployees} />;
        case "departments": return <DepartmentsPanel user={user} onLogout={logout} setEmployees={setEmployees} />;
        default: return <AdminOverview onNavigate={nav} user={user} projects={projects} reports={reports} projectItems={projectItems} employees={employees} onStatusChange={handleStatusChange} onEditProject={handleEditProject} onLogout={logout} setEmployees={setEmployees} profilePhotoPreview={profilePhotoPreview} setProfilePhotoPreview={setProfilePhotoPreview} />;
      }
    }
    if (role === "tl") {
      switch (section) {
        case "tl-submit": return <ReportSubmission employee={user} user={user} projects={projects} reports={reports} projectItems={projectItems} onSubmit={handleReportSubmit} onLogout={logout} isTL={true} setEmployees={setEmployees} />;
        case "tl-projects": return <TLProjectsPanel user={user} projects={projects} reports={reports} projectItems={projectItems} employees={employees} onEditProject={handleEditProject} onLogout={logout} setEmployees={setEmployees} />;
        case "tl-announce": return <TLAnnouncementPanel user={user} announcements={announcements}
          ...
          }} onLogout={logout} setEmployees={setEmployees} />;
        case "tl-history": return <EmployeeHistory employee={user} user={user} reports={reports} onLogout={logout} setEmployees={setEmployees} />;
        default: return <ReportSubmission employee={user} user={user} projects={projects} reports={reports} projectItems={projectItems} onSubmit={handleReportSubmit} onLogout={logout} isTL={true} setEmployees={setEmployees} />;
      }
    }
    switch (section) {
      case "submit": return <ReportSubmission employee={user} user={user} projects={projects} reports={reports} projectItems={projectItems} onSubmit={handleReportSubmit} onLogout={logout} announcements={announcements} setEmployees={setEmployees} />;
      case "history": return <EmployeeHistory employee={user} user={user} reports={reports} onLogout={logout} announcements={announcements} setEmployees={setEmployees} />;
      case "announcements": return <EmployeeAnnouncementsPanel user={user} announcements={announcements} onLogout={logout} setEmployees={setEmployees} />;
      default: return <ReportSubmission employee={user} user={user} projects={projects} reports={reports} projectItems={projectItems} onSubmit={handleReportSubmit} onLogout={logout} announcements={announcements} setEmployees={setEmployees} />;
    }
  }
    ...
    <>
      ...
      {error && (
        <div style={{
          ...
          padding: "12px 20px", zIndex: 99999, display: "flex",
          alignItems: "center", justifyContent: "space-between",
          boxShadow: "0 2px 8px rgba(0,0,0,0.1)"
        }}>
          <span style={{ color: "#dc2626", fontWeight: 600 }}>⚠️ {error}</span>
          <button onClick={() => setError(null)} style={{
            background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#dc2626"
          }}>×</button>
        </div>
      )}
      {loading
        ...
        : !role
          ...
          : <div className="d-flex vh-100 overflow-hidden" style={{ background: "linear-gradient(145deg,#e8f4ff 0%,#f0f8ff 60%,#e8f4ff 100%)", fontFamily: "'Nunito',system-ui,sans-serif" }}>
            <Sidebar role={role} active={section} onNav={nav} user={{ ...user, avatar: profilePhotoPreview || user?.avatar }} onLogout={logout}
            ...
            <div className="flex-fill overflow-auto main-content" style={{ padding: "clamp(12px, 3vw, 16px)", paddingBottom: (role === "employee" || role === "tl" || role === "admin") ? 72 : 16 }}>
              <div style={{
                ...
                width: "100%", maxWidth: "min(1400px, 100%)", margin: "0 auto",
                boxShadow: "0 4px 32px rgba(37,99,235,.08)", padding: "clamp(16px, 4vw, 30px)"
              }}>
                {content()}
              </div>
            </div>
            ...
            {/* Bottom Nav */}
            <BottomNav role={role} active={section} onNav={nav} unreadAnnouncements={0} />
